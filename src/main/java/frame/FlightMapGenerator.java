...
public class FlightMapGenerator {
    ...
    static {
        ...
        cityCoordinates.put("山东", new double[]{36.6512, 117.1201});
    ...
    public void generateMap(List<Order> flights) throws IOException {
        ...
        for (Order flight : flights) {
            ...
            // 比较起飞时间是否已经过期
            boolean isExpired = flight.getFlightId().getStartTime().isBefore(now);
            String color = isExpired ? "gray" : "pink"; // 过期航班设为灰色，其他设为粉色

            htmlContent.append(String.format(
                    "{start: [%f, %f], end: [%f, %f], startCity: '%s', endCity: '%s', info: '航班号: %s, 出发城市: %s, 到达城市: %s, 起飞时间: %s, 降落时间: %s', color: '%s'},\n",
                    startCoords[0], startCoords[1], endCoords[0], endCoords[1],
                    startCity, endCity,
                    flight.getFlightId().getFlightName(),
                    startCity,
                    endCity,
                    flight.getFlightId().getStartTime().format(formatter),
                    flight.getFlightId().getArrivalTime().format(formatter),
                    color
            ));
        }
        ...
        htmlContent.append("""
                        ...
                        flights.forEach(function(flight) {
                            ...
                            var curvedRoute = L.curve(latlngs, {color: flight.color, weight: 2})
                                .on('mouseover', function(e) {
                                    this.setStyle({color: flight.color === 'gray' ? 'darkgray' : 'blue', weight: 4});
                                    var popup = L.popup()
                                        .setLatLng(e.latlng)
                                ...
                                .on('mouseout', function(e) {
                                    this.setStyle({color: flight.color, weight: 2});
                                    map.closePopup();
                                })
                                .addTo(map);
                        });
    ...
    public static void main(String[] args) throws IOException {
        ...
        // 示例订单（假设 Order 构造函数中包含订单号、乘客ID、航班号等信息）
